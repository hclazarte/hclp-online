...
const MedicosComponent = forwardRef(({ modo, medico, setMedico }, ref) => {
  ...
  const todasLasEspecialidades = [
    ...
    },
    {
      ...
      descripcion:
        'Especialista en intervenciones quirúrgicas de diferentes tipos.'
    },
    {
      id: 3,
      nombre: 'Psiquiatría',
      descripcion:
        'Especialista en trastornos mentales y comportamiento humano.'
    }
  ]
  ...
  const validateMedico = (medico) => {
    ...
    if (
      !medico.registro_profesional ||
      medico.registro_profesional.trim() === ''
    ) {
                ...
                return (
                  <label key={esp.id} className='flex items-center space-x-2'>
                    <input
                      ...
                      checked={checked}
                      onChange={(e) =>
                        handleEspecialidadesChange(esp, e.target.checked)
                      }
                      disabled={modo === 'navegacion'}
                    />
